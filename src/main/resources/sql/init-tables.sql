...
-- 1. 푸시 메시지 마스터 테이블
CREATE TABLE SC_PT.PUSH_MSG_MASTER (
...
-- 2. 푸시 발송 대상 정보 테이블
CREATE TABLE SC_PT.PUSH_SEND_TARGET_INFO (
    SEQ NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    PUSH_MSG_SEQ NUMBER,
...
-- 3. 푸시 발송 대기 테이블
CREATE TABLE SC_PT.PUSH_SEND_STBY (
    SEQ NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    PUSH_MSG_SEQ NUMBER,
    TARGET_EMPID VARCHAR2(50),
...
-- 4. 사용자 디바이스 정보 테이블
CREATE TABLE SC_PT.USER_DEVICE_INFO (
...
-- 5. 푸시 발송 성공 결과 테이블
CREATE TABLE SC_PT.PUSH_SUCCESS_RESULT (
    SEQ NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
-- 6. 푸시 발송 실패 결과 테이블
CREATE TABLE SC_PT.PUSH_FAIL_RESULT (
    SEQ NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    PUSH_SEND_STBY_SEQ NUMBER,
    PUSH_MSG_SEQ NUMBER,
    DEVICE_TOKEN VARCHAR2(1000),
...
-- 인덱스 생성
CREATE INDEX IDX_PUSH_MSG_MASTER_PRC_FLAG ON SC_PT.PUSH_MSG_MASTER(PRC_FLAG, RSV_YN);
CREATE INDEX IDX_PUSH_SEND_TARGET_MSG_SEQ ON SC_PT.PUSH_SEND_TARGET_INFO(PUSH_MSG_SEQ);
CREATE INDEX IDX_PUSH_SEND_STBY_PRC_FLAG ON SC_PT.PUSH_SEND_STBY(PRC_FLAG);
CREATE INDEX IDX_USER_DEVICE_EMP_ID ON SC_PT.USER_DEVICE_INFO(EMP_ID, USE_YN);
